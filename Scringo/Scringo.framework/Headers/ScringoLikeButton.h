...
typedef NS_ENUM(NSUInteger, ScringoAppLikeObjectType) {
    ...
    /** Link Like Object type */
    SCRINGO_APP_LIKE_LINK,
    /** Topic Like Object type */
    SCRINGO_APP_LIKE_TOPIC,
    /** Tweet Like Object type */
    SCRINGO_APP_LIKE_TWEET
};
 ...
 */
-(void)likeObject:(ScringoLikeObject *)likeObject withData:(NSString *)data newMode:(BOOL)mode numberOfLikes:(int)numberOfLikes;
@end
...
/** Update Button with details.
 ...
 @param data The like object data, usually an image or video URL, HTML, Link or plain text.
...
/** Update Button with Topic Id.
 
 Set the internal like object with Topic details.
 
 @param topicId The Topic Id.
 */
-(void)updateTopicLikeObject:(int)topicId;

/** Update Button with Chat Room details.
 
 Set the internal like object with the given Chat Room details.
 
 @param chatRoom The Chat Room to attach the like object to.
 */
-(void)updateLikeObjectFromChatRoom:(ScringoChatRoom *)chatRoom;
...
/** Pause Like Button server communications.
 
 Stops any on going activity between the button and the server. Requires an update Method to resume communications.
 */
-(void)pause;
